...
const graph: { [key: string]: NodoEstacion[] } = {
...
let nodes: string[] = Object.keys(graph);
let currentNodeIndex: number = 0;

function deleteEstacion(graph: { [key: string]: NodoEstacion[] }, keyToDel: string): void {
...
function updateUI(): void {
    ...
    if (stationsList) {
        ...
        graph[currentNode]?.forEach(estacion => {
...
function addEventListeners(): void {
    ...
    upButton?.addEventListener('click', () => moveNode('up'));
    downButton?.addEventListener('click', () => moveNode('down'));
    saveButton?.addEventListener('click', (event) => saveEstacion(event));
    
    deleteButton?.addEventListener('click', () => deleteEstacion(graph,nodes[currentNodeIndex]));
    
}

function saveEstacion(event: Event): void {
    event.preventDefault(); // Prevent form submission

    // Retrieve values from form
    const nombre = (document.getElementById('nombre') as HTMLInputElement).value;
    const lineas = (document.getElementById('lineas') as HTMLInputElement).value.split(','); // Split by comma
    const adyacentes = (document.getElementById('adyacencias') as HTMLInputElement).value.split(','); // Estacion anterior y posterior
    const direccion = (document.getElementById('direccion') as HTMLInputElement).value;
    const horario = (document.getElementById('horario') as HTMLInputElement).value;
    const extra1 = (document.getElementById('extra1') as HTMLInputElement).value;
    const extra2 = parseInt((document.getElementById('extra2') as HTMLInputElement).value, 10);
...
function modifyEstacion(
    graph: { [key: string]: NodoEstacion[] },
    newkey: string,
    nuevaEstacion: NodoEstacion,
    afterEstacion?: string,
    beforeEstacion?: string
): void {
    const newGraph: { [key: string]: NodoEstacion[] } = {};
...
function findEstacionInGraph(
    graph: { [key: string]: NodoEstacion[] },
    key: string,
    estacionName: string
): NodoEstacion | undefined {
...
function moveNode(direction: 'up' | 'down'): void {
